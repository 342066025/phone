...
export async function commitStagedActions() {
    ...
    playerActionsToCommit.forEach(action => {
        ...
        switch(action.type) {
            ...
            case 'new_forum_reply': {
                ...
                if (post) {
                    textPrompt += `- 在论坛"${action.boardName}"板块回复了帖子"${post.title}"，回复内容为"${action.content}"\\n`;
                }
                break;
            }
        }
    ...
    if (hasActionsForAI) {
        // Persist all messages to worldbook
        for (const msgData of finalMessagesToPersist) {
            await _updateWorldbook(PhoneSim_Config.WORLD_DB_NAME, dbData => {
                const contact = dbData[msgData.contactId];
                if (contact && contact.app_data && contact.app_data.WeChat) {
                    contact.app_data.WeChat.messages.push(msgData.message);
                }
                return dbData;
            });
        }
        ...
        // Refresh data and UI
        await DataHandler.fetchAllData();
        UI.rerenderCurrentView({ chatUpdated: true, momentsUpdated: true, forumUpdated: true, liveCenterUpdated: true });
