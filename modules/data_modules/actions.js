...
// Cache for character-specific lorebooks
let characterLorebookCache = new Map();

// Helper function to get current character name for prompts
function getCurrentCharacterName() {
    const currentCharacter = PhoneSim_State.getCurrentCharacter();
    return currentCharacter ? currentCharacter.name : '{{user}}';
}
...
export function clearLorebookCache() {
    ...
    characterLorebookCache.clear();
}

export async function getOrCreateCharacterLorebook(characterId = null) {
    // If no characterId provided, use current character from state
    const targetCharacterId = characterId || PhoneSim_State.currentCharacterId;
    
    if (!targetCharacterId) {
        console.error("[Phone Sim] No character ID provided and no current character set");
        return null;
    }

    // Check cache first
    if (characterLorebookCache.has(targetCharacterId)) {
        return characterLorebookCache.get(targetCharacterId);
    }

    const lorebookName = `${PhoneSim_Config.CHARACTER_LOREBOOK_PREFIX}${targetCharacterId}`;
    ...
    if (!allLorebooks.includes(lorebookName)) {
        const initialEntries = PhoneSim_Config.getInitialCharacterEntries(targetCharacterId);
        await TavernHelper_API.createOrReplaceWorldbook(lorebookName, initialEntries);
    }
    ...
    characterLorebookCache.set(targetCharacterId, lorebookName);
    return lorebookName;
}
...
export async function _updateCharacterWorldbook(dbName, updaterFn, characterId = null) {
    const lorebookName = await getOrCreateCharacterLorebook(characterId);
    if (!lorebookName) {
        console.error("[Phone Sim] Cannot update character worldbook: Could not get or create a dedicated lorebook.");
        return false;
    }
    ...
    try {
        ...
        if (dbIndex === -1) {
            console.warn(`[Phone Sim] Entry '${dbName}' not found in character lorebook '${lorebookName}'. Creating new entry.`);
            const newEntry = { 
                name: dbName, 
                content: defaultContent, 
                enabled: false,
                comment: `Managed by Phone Simulator Plugin for character ${characterId || PhoneSim_State.currentCharacterId}. Do not edit manually.` 
            };
        ...
        console.error(`[Phone Sim] Error updating character worldbook (${dbName}):`, er);
        return false;
    }
}

export async function _updateWorldbook(dbName, updaterFn) {
    // For backward compatibility, use character-specific worldbook if current character exists
    if (PhoneSim_State.currentCharacterId) {
        return await _updateCharacterWorldbook(dbName, updaterFn);
    }
...
export async function initiateVoiceCall(contactId) {
    ...
    const prompt = `(系统提示：${characterName}向${contactName}发起了微信语音通话...)`;
...
export async function initiatePhoneCall(callTarget) {
    const characterName = getCurrentCharacterName();
    const prompt = `(系统提示：${characterName}正在呼叫${callTarget.name}的电话...)`;
    await TavernHelper_API.triggerSlash(`/setinput ${JSON.stringify(prompt)}`);
    SillyTavern_Context_API.generate();
    UI.closeCallUI();
...
export function stagePlayerMessage(contactId, messageContent, replyingToUid = null, descriptionForAI = null) {
    ...
    // Use current character ID if available, otherwise fall back to PLAYER_ID
    const senderId = PhoneSim_State.currentCharacterId || PhoneSim_Config.PLAYER_ID;
...
export async function commitStagedActions() {
    ...
    const characterName = getCurrentCharacterName();
    let textPrompt = `(系统提示：${characterName}刚刚在手机上进行了如下操作：\\n`;
...
// 角色间通信功能
export async function addCharacterAsContact(targetCharacterId, sourceCharacterId = null) {
    const sourceId = sourceCharacterId || PhoneSim_State.currentCharacterId;
    const sourceCharacter = PhoneSim_State.characters[sourceId];
    const targetCharacter = PhoneSim_State.characters[targetCharacterId];
    
    if (!sourceCharacter || !targetCharacter) {
        console.error('[Phone Sim] Invalid character IDs for cross-character communication');
        return false;
    }

    // 获取目标角色的世界书
    const targetLorebookName = await getOrCreateCharacterLorebook(targetCharacterId);
    if (!targetLorebookName) {
        console.error('[Phone Sim] Failed to get target character lorebook');
        return false;
    }

    // 在目标角色的联系人列表中添加源角色
    await _updateCharacterWorldbook(PhoneSim_Config.WORLD_DB_NAME, dbData => {
        if (!dbData[sourceId]) {
            dbData[sourceId] = {
                profile: { 
                    nickname: sourceCharacter.name, 
                    note: sourceCharacter.name,
                    has_custom_avatar: !!sourceCharacter.avatar
                },
                app_data: { WeChat: { messages: [] } },
                moments: []
            };
    ...
    // 如果源角色有头像，也添加到目标角色的头像数据库
    if (sourceCharacter.avatar) {
        await _updateCharacterWorldbook(PhoneSim_Config.WORLD_AVATAR_DB_NAME, avatarData => {
            avatarData[sourceId] = sourceCharacter.avatar;
            return avatarData;
        }, targetCharacterId);
    }

    // 更新目标角色的联系人目录
    await _updateCharacterWorldbook(PhoneSim_Config.WORLD_DIR_NAME, dirData => {
        if (!dirData.contacts) dirData.contacts = {};
        dirData.contacts[sourceCharacter.name] = sourceId;
        return dirData;
    }, targetCharacterId);
...
export async function sendCrossCharacterMessage(targetCharacterId, messageContent, sourceCharacterId = null) {
    const sourceId = sourceCharacterId || PhoneSim_State.currentCharacterId;
    const sourceCharacter = PhoneSim_State.characters[sourceId];
    
    if (!sourceCharacter) {
        console.error('[Phone Sim] Invalid source character ID');
        return false;
    }

    // 确保目标角色的联系人列表中有源角色
    await addCharacterAsContact(targetCharacterId, sourceId);

    // 在目标角色的数据库中添加消息
    await _updateCharacterWorldbook(PhoneSim_Config.WORLD_DB_NAME, dbData => {
        const contact = dbData[sourceId];
        if (contact && contact.app_data && contact.app_data.WeChat) {
            const message = {
                uid: `cross_char_${Date.now()}_${Math.random()}`,
                timestamp: new Date().toISOString(),
                sender_id: sourceId,
                content: messageContent
            };
            contact.app_data.WeChat.messages.push(message);
...
export async function enableCrossCharacterCommunication() {
    // 为所有现有角色互相添加为联系人
    const characterIds = Object.keys(PhoneSim_State.characters);
    
    for (const sourceId of characterIds) {
        for (const targetId of characterIds) {
            if (sourceId !== targetId) {
                await addCharacterAsContact(targetId, sourceId);
            }
        }
    }
    
    console.log('[Phone Sim] Cross-character communication enabled for all characters');
    return true;
}
